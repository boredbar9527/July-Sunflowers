...
// Manual SKU aliases: catalog SKU -> the SKU token used in the source filename.
// For products whose photo exists but is filed under a different code
// (e.g. Kraft take-out boxes are named "Carton 1#" not "CY-KTB1").
const ALIAS = {
  "CY-KTB1": "Carton 1#",
  "CY-KTB2": "Carton 2#",
  "CY-KTB3": "Carton 3#",
...
for (const p of products) {
  const src =
    (ALIAS[p.sku] && skuMap.get(norm(ALIAS[p.sku]))) ||
    skuMap.get(norm(p.sku)) ||
    detailMap.get(alnum(p.name)) ||
    null;
  if (!src) {
    missing.push(p);
    continue;
